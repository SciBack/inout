...
class TestUpsertBasico:
    def test_crea_persona_e_indexa_credenciales(self, db):
        upsert_person(
            db,
            rec("ldap:40390492", {"cardnumber": "201913085", "dni": "40390492"}, full_name="Ada"),
            source="ldap",
        )
    ...
    def test_es_idempotente(self, db):
        r = rec("ldap:40390492", {"cardnumber": "201913085", "dni": "40390492"}, full_name="Ada")
    ...
    def test_actualiza_campos_informados(self, db):
        upsert_person(db, rec("k", {"dni": "1"}, full_name="Ada", faculty="FIA"), source="ldap")
        upsert_person(db, rec("k", {"dni": "1"}, full_name="Ada Lovelace"), source="ldap")
    ...
    def test_no_borra_data_previa_con_none(self, db):
        ...
        upsert_person(db, rec("k", {"dni": "1"}, full_name="Ada", faculty="FIA"), source="ldap")
        upsert_person(db, rec("k", {"dni": "1"}, full_name="Ada", faculty=None), source="ldap")
...
class TestBusqueda:
    ...
    def test_por_tipo_y_valor(self, db):
        ...
        assert find_person_by_identifier(db, "dni", "201913085") is None
...
class TestReconciliacionCrossProveedor:
    ...
    def test_misma_persona_dos_proveedores_una_sola_fila(self, db):
        ...
        upsert_person(
            db,
            rec("koha:9999", {"dni": "40390492"}, full_name="Ada L."),
            source="koha_db",
        )
        assert db.query(Person).count() == 1
    ...
    def test_el_person_key_no_se_reasigna(self, db):
        ...
        upsert_person(db, rec("ldap:40390492", {"dni": "40390492"}, full_name="Ada"), source="ldap")
        upsert_person(db, rec("koha:9999", {"dni": "40390492"}, full_name="Ada"), source="koha_db")
        p = db.query(Person).one()
    ...
    def test_las_credenciales_nuevas_se_suman_a_la_fila_existente(self, db):
        ...
        upsert_person(
            db,
            rec("koha:9999", {"dni": "40390492", "cardnumber": "201913085"}, full_name="Ada"),
            source="koha_db",
        )
    ...
    def test_personas_distintas_no_se_mezclan(self, db):
        ...
        upsert_person(db, rec("ldap:2", {"dni": "2", "cardnumber": "20"}, full_name="Grace"), source="ldap")
